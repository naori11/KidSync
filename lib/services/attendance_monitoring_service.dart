...
class AttendanceMonitoringService {
  ...
  // Absence thresholds
  static const int PARENT_NOTIFICATION_THRESHOLD =
      5; // Parent notified after 5 absences
  static const int ESCALATION_DAYS =
      3; // Escalate if no response after 3 school days
  ...
  Future<Map<String, dynamic>> getStudentAttendanceStats({
    ...
    try {
      ...
      final notificationHistory = await getAttendanceNotificationHistory(
        studentId,
      );
      bool hasParentNotificationSent = notificationHistory.any(
        (notification) => [
          'attendance_alert',
          'attendance_ticket',
      ...
      if (hasParentNotificationSent) {
        ...
        if (recentNotification.isNotEmpty &&
            recentNotification['created_at'] != null) {
          lastNotificationDate = DateTime.tryParse(
            recentNotification['created_at'],
          );
        }
      }
      ...
      for (final record in recordsList) {
        ...
        if (date.isNotEmpty)
          attendanceMap[date] = Map<String, dynamic>.from(record);
      }
      ...
      while (currentDate.isBefore(
        defaultEndDate.add(const Duration(days: 1)),
      )) {
      ...
      while (checkDate.isAfter(
        defaultStartDate.subtract(const Duration(days: 1)),
      )) {
      ...
      if (!hasParentNotificationSent) {
        isUrgentIssue =
            absentDays >= PARENT_NOTIFICATION_THRESHOLD ||
        ...
        while (checkAfter.isAfter(lastNotificationDate)) {
          if (classDays.isEmpty || isClassDay(checkAfter)) {
            final dateStr =
                "${checkAfter.year.toString().padLeft(4, '0')}-${checkAfter.month.toString().padLeft(2, '0')}-${checkAfter.day.toString().padLeft(2, '0')}";
            bool isAbsentAfter = false;
            if (attendanceMap.containsKey(dateStr)) {
              isAbsentAfter =
                  (attendanceMap[dateStr]!['status'] ?? 'Absent') == 'Absent';
  ...
  Future<List<Map<String, dynamic>>> getStudentsWithAttendanceIssues({
    ...
    try {
      ...
      for (final section in sections) {
        ...
        for (final student in students) {
          ...
          if (stats['consecutiveAbsences'] >= 3 &&
              (!stats['hasParentNotificationSent'] || stats['isUrgentIssue'])) {
      ...
      studentsWithIssues.sort((a, b) {
        ...
        return (bStats['absentDays'] as int).compareTo(
          aStats['absentDays'] as int,
        );
      });
  ...
  Future<bool> sendParentAbsenceNotification({
    ...
    try {
      ...
      if (consecutiveAbsences > 0) {
        message +=
            '$studentName has been absent for $consecutiveAbsences consecutive school days. ';
      }

      message += 'Total absences: $absentCount. ';
      message +=
          'Please contact $teacherName ($sectionName) to discuss this matter. ';
      ...
      if (notifications.isNotEmpty) {
        ...
        if (parentPhones.isNotEmpty) {
          ...
          try {
            print(
              'AttendanceMonitoringService: enqueuing SMS for absence alert -> recipients=${parentPhones.length}, preview="${smsMessage.substring(0, smsMessage.length > 80 ? 80 : smsMessage.length)}"',
            );
          } catch (_) {}
  ...
  Future<List<Map<String, dynamic>>> checkForFollowUps() async {
    try {
      ...
      final cutoffDate = TimeUtils.nowPST().subtract(
        Duration(days: ESCALATION_DAYS),
      );
      ...
      for (final notification in overdueNotifications) {
        ...
        followUpsNeeded.add({
          ...
          'days_overdue':
              DateTime.now()
                  .difference(DateTime.parse(notification['created_at']))
                  .inDays,
          'current_stats': stats,
        });
      }
  ...
  Future<List<Map<String, dynamic>>> getAttendanceNotificationHistory(
    int studentId,
  ) async {
    try {
  ...
  Future<Map<String, dynamic>> getNotificationEffectiveness({
    ...
    try {
      ...
      final daysSinceNotification =
          TimeUtils.nowPST().difference(notificationDate).inDays;
  ...
  Future<bool> hasParentResponded(int studentId) async {
    try {
      ...
      return response != null &&
          (response['read_at'] != null || response['is_read'] == true);
    } catch (e) {
  ...
  Future<bool> _sendFollowUpNotification(
    Map<String, dynamic> originalNotification,
  ) async {
    try {
      ...
      String message =
          'This is a follow-up regarding $studentName\'s attendance concerns. ';
      message +=
          'We previously notified you about attendance issues, but have not received a response. ';
      message += 'Current status: ${stats['absentDays']} total absences. ';
      message +=
          'Please contact the school immediately to discuss this matter. ';
      message +=
          'If this issue is not addressed promptly, it will be escalated to administration.';
      ...
      if (notifications.isNotEmpty) {
        ...
        try {
          ...
          if (parentPhones.isNotEmpty) {
            ...
            try {
              print(
                'AttendanceMonitoringService: enqueuing SMS for follow-up -> recipients=${parentPhones.length}, preview="${smsMsg.substring(0, smsMsg.length > 80 ? 80 : smsMsg.length)}"',
              );
            } catch (_) {}
          ...
          print(
            'SMS enqueue error (attendance_monitoring._sendFollowUpNotification): $e',
          );
        }
  ...
  Future<bool> markAttendanceIssueResolved({
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        'message':
            'Attendance concern manually resolved for student ID $studentId. Notes: ${resolutionNotes ?? "Teacher spoke with parent at school"}',
      ...
      await supabase.from('scan_records').insert({
        ...
        'rfid_uid':
            'teacher_resolution_${studentId}_${now.millisecondsSinceEpoch}',
        'scan_time': TimeUtils.formatForDatabase(now),
        'action': 'attendance_resolution',
        'notes':
            'Attendance issue manually resolved by teacher - resets consecutive absence count',
      });
      ...
      await _logNotificationInDatabase(
        ...
        details: {
          ...
          'resolution_notes':
              resolutionNotes ?? 'Teacher spoke with parent at school',
      ...
      print(
        'Attendance issue manually resolved for student $studentId - consecutive absences reset',
      );
      return true;
    } catch (e) {
    ...
    try {
      ...
      if (notifications.isNotEmpty) {
        ...
        try {
          ...
          print(
            'SMS enqueue error (attendance_monitoring.sendCustomParentNotification): $e',
          );
        }
  ...
  Future<void> _logNotificationInDatabase({
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        'message':
            'System log: $notificationType for student ID $studentId. Details: ${details.toString()}',
    ...
    try {
      ...
      for (final teacher in teachers) {
        ...
        for (final studentData in studentsWithIssues) {
          ...
          final hasParentNotificationSent =
              stats['hasParentNotificationSent'] as bool;
          ...
          if (consecutiveAbsences >= 5 &&
              ...
              (existingNotifications.isEmpty ||
                  DateTime.now()
                          .difference(
                            DateTime.parse(
                              existingNotifications.first['created_at'],
                            ),
                          )
                          .inDays >=
                      2)) {
