...
class VerificationService {
  ...
  Future<bool> createVerificationRequest({
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        for (final parentData in parentResponse) {
          ...
          if (parent != null) {
            ...
            try {
              ...
              if (existingVerification == null) {
                await supabase
                    ...
                    .insert({
                      ...
                      'status': 'pending',
                      'created_at': TimeUtils.formatForDatabase(
                        TimeUtils.nowPST(),
                      ),
                    })
                    .timeout(Duration(seconds: 10));
                ...
                if (parentUserId != null) {
                  try {
                    await notificationService.sendCustomNotification(
                      ...
                      message:
                          'Please verify the $eventType of $studentName by $driverName at ${TimeUtils.formatTimeForDisplay(eventTime)}.',
                      type: '${eventType}_verification_request',
                      extraData: {'student_id': studentId.toString()},
                    );

                    print(
                      '✅ Verification request notification sent to parent $parentId',
                    );
                  } catch (notificationError) {
                    print(
                      '❌ Error sending notification to parent $parentId: $notificationError',
                    );
                  }
                }
              ...
              print(
                'Error creating verification for parent $parentId: $parentError',
              );
        ...
        return successCount >
            0; // Success if at least one verification was created
      } catch (e) {
        retryCount++;
        ...
        if (retryCount >= maxRetries) {
          print(
            'Failed to create verification request after $maxRetries attempts',
          );
          return false;
        } else {
          // Exponential backoff
  ...
  Future<bool> confirmVerification(
    ...
    try {
      ...
      await supabase
          ...
          .update({
            'status': 'confirmed',
            'parent_response_time': TimeUtils.formatForDatabase(
              TimeUtils.nowPST(),
            ),
      ...
      if (student != null && driver != null && parent != null) {
        ...
        try {
          ...
          if (notificationSent) {
            ...
            print(
              '❌ Driver approval notification failed to send (returned false)',
            );
          }
        } catch (notificationError) {
          print(
            '❌ Error sending driver approval notification: $notificationError',
          );
        }
      } else {
      ...
      if (student != null && driver != null && parent != null) {
        ...
        try {
          ...
          if (notificationSent) {
            ...
            print(
              '❌ Driver denial notification failed to send (returned false)',
            );
          }
        } catch (notificationError) {
          print(
            '❌ Error sending driver denial notification: $notificationError',
          );
        }
      } else {
  ...
  Future<void> sendReminders() async {
    try {
      ...
      final cutoffTime = TimeUtils.nowPST().subtract(
      ...
      for (final verification in pendingVerifications) {
        ...
        await supabase
            ...
            .update({
              ...
              'last_reminder_sent': TimeUtils.formatForDatabase(
                TimeUtils.nowPST(),
              ),
            })
